...
heat_map(x) +
  labs(title = 'Round 9: interpolated positivity')
```
...
### Round-specific raw numbers figure
...
x <- x %>%
  gather(key, value, percent_positive:Tests )
...
```{r}
x <- x %>%
  dplyr::select(round, positives, key, value)
prettify(x)
```
